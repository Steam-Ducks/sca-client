...
const fetchKPIsMock = vi.fn().mockResolvedValue(KPIS_MOCK)
...
// Mock fetch global for fetchTableData (/api/consolidated/)
...
describe('DashboardView.vue', () => {
  ...
  it('CT02: displays total materials cost from API', async () => {
    ...
    const cards = wrapper.findAll('.metric-card')
    const card = cards.find((c) => c.text().includes('Custo Total de Materiais'))
    expect(card).toBeDefined()
  ...
  it('CT03: displays total technical hours cost from API', async () => {
    ...
    const cards = wrapper.findAll('.metric-card')
    const card = cards.find((c) => c.text().includes('Custo Total de Horas Técnicas'))
    expect(card).toBeDefined()

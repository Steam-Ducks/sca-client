...
function destroyAll() {
  ;[chartCustoPrograma, chartComparativo, chartTemporal, chartTopProjetos]
    .forEach(c => c?.destroy())
  chartCustoPrograma = chartComparativo = chartTemporal = chartTopProjetos = null
}
...
function buildCharts(data: DashboardRow[]) {
  ...
  const ctxProg = (document.getElementById('chartCustoPrograma') as HTMLCanvasElement)?.getContext('2d')
  if (ctxProg) {
  ...
  const matPorProjeto = projetos.map(p => data.filter(r => r.projeto === p).reduce((s, r) => s + r.custoMateriais, 0))
  const horasPorProjeto = projetos.map(p => data.filter(r => r.projeto === p).reduce((s, r) => s + r.custoHoras, 0))

  const ctxComp = (document.getElementById('chartComparativo') as HTMLCanvasElement)?.getContext('2d')
  if (ctxComp) {
    chartComparativo = new Chart(ctxComp, {
      ...
      data: {
        labels: projetos,
        datasets: [
          { label: 'Materiais', data: matPorProjeto, backgroundColor: 'rgba(77,143,255,0.85)', borderRadius: 4, borderSkipped: false },
          { label: 'Horas Técnicas', data: horasPorProjeto, backgroundColor: 'rgba(45,212,160,0.85)', borderRadius: 4, borderSkipped: false },
        ],
      },
      options: {
        ...baseOptions('y'),
  ...
  const ctxT = (document.getElementById('chartTemporalDash') as HTMLCanvasElement)?.getContext('2d')
  if (ctxT) {
  ...
  const topProjetos = groupBy(data, r => r.projeto, r => r.custoTotal).slice(0, 10)
  const ctxTP = (document.getElementById('chartTopProjetos') as HTMLCanvasElement)?.getContext('2d')
  if (ctxTP) {
...
function updateCharts(data: DashboardRow[]) {
  ...
  if (chartComparativo) {
    ...
    chartComparativo.data.datasets[0].data = projetos.map(p => data.filter(r => r.projeto === p).reduce((s, r) => s + r.custoMateriais, 0))
    chartComparativo.data.datasets[1].data = projetos.map(p => data.filter(r => r.projeto === p).reduce((s, r) => s + r.custoHoras, 0))

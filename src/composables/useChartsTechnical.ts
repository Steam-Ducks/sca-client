...
const baseScales = (axis: 'x' | 'y' = 'x') => ({
  x: {
    grid:  { color: axis === 'x' ? gridColor : 'transparent', drawBorder: false },
    ticks: { color: textColor, font: { family: MONO, size: 11 } },
  },
  y: {
    grid:  { color: axis === 'y' ? gridColor : 'transparent', drawBorder: false },
    ticks: { color: text2Color, font: { family: FONT, size: 11 }, maxTicksLimit: 8 },
  },
})
...
function destroyAll() {
  ;[chartHorasProjeto, chartCustoProjeto, chartCustoColaborador, chartTemporal]
    .forEach(c => c?.destroy())
  chartHorasProjeto = chartCustoProjeto = chartCustoColaborador = chartTemporal = null
}
...
function buildCharts(data: HoraRow[]) {
  ...
  const ctxHP = (document.getElementById('chartHorasProjeto') as HTMLCanvasElement)?.getContext('2d')
  if (ctxHP) {
  ...
  const ctxCP = (document.getElementById('chartCustoProjeto') as HTMLCanvasElement)?.getContext('2d')
  if (ctxCP) {
  ...
  const ctxCC = (document.getElementById('chartCustoColaborador') as HTMLCanvasElement)?.getContext('2d')
  if (ctxCC) {
  ...
  const ctxT = (document.getElementById('chartTemporal') as HTMLCanvasElement)?.getContext('2d')
  if (ctxT) {
...
function updateCharts(data: HoraRow[]) {
  // Horas por projeto
  const horasProjeto = groupBy(
    data, r => `${r.programa} - ${r.projeto}`, r => r.horas,
  ).slice(0, 8)
  ...
  // Custo por projeto
  const custoProjeto = groupBy(
    data, r => `${r.programa} - ${r.projeto}`, r => r.custoTotal,
  ).slice(0, 8)
  ...
  const custoColab = groupBy(
    data, r => r.colaborador, r => r.custoTotal,
  ).slice(0, 10)
  ...
  // Temporal
  const temporalMap: Record<string, number> = {}
  data.forEach(r => { temporalMap[r.periodo] = (temporalMap[r.periodo] || 0) + r.horas })
  const periodos = Object.keys(temporalMap).sort()
  if (chartTemporal) {

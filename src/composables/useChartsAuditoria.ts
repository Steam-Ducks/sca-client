...
function destroyAll() {
  ;[chartStatusPeriodo, chartPorTipo, chartPorResponsavel, chartTemporal]
    .forEach(c => c?.destroy())
  chartStatusPeriodo = chartPorTipo = chartPorResponsavel = chartTemporal = null
}
...
function buildCharts(data: AuditoriaRow[]) {
  ...
  const porStatus = groupBy(data, r => r.status, () => 1)
  ...
  const ctxSP = (document.getElementById('chartStatusPeriodo') as HTMLCanvasElement)?.getContext('2d')
  if (ctxSP) {
    chartStatusPeriodo = new Chart(ctxSP, {
      ...
      data: {
        ...
        datasets: [{
          ...
          backgroundColor: porStatus.map(([l]) => statusColors[l] || 'rgba(155,127,255,0.85)'),
          borderRadius: 4,
          borderSkipped: false,
        }],
      },
      options: {
        ...
        scales: {
          ...
          y: {
            ...
            ticks: { color: textColor, font: { family: MONO, size: 11 }, stepSize: 1 },
          },
        },
      },
    })
  ...
  const porTipo = groupBy(data, r => r.tipo, () => 1)
  const ctxTipo = (document.getElementById('chartPorTipo') as HTMLCanvasElement)?.getContext('2d')
  if (ctxTipo) {
  ...
  const porResp = groupBy(data, r => r.responsavel, () => 1).slice(0, 10)
  const ctxResp = (document.getElementById('chartPorResponsavel') as HTMLCanvasElement)?.getContext('2d')
  if (ctxResp) {
  ...
  const ctxT = (document.getElementById('chartTemporalAud') as HTMLCanvasElement)?.getContext('2d')
  if (ctxT) {
...
function updateCharts(data: AuditoriaRow[]) {
  ...
  if (chartStatusPeriodo) {
    ...
    chartStatusPeriodo.data.datasets[0].backgroundColor = porStatus.map(([l]) => statusColors[l] || 'rgba(155,127,255,0.85)')
